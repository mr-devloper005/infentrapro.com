...
const cleanSummaryText = (value: string) => value
  ...
  .replace(/\s+/g, ' ')
  .trim()
const getSummary = (post: SitePost) => cleanSummaryText(post.summary || asText(getContent(post).description) || asText(getContent(post).excerpt) || asText(getContent(post).body))
...
export async function EditableTaskArchiveRoute({
  ...
  if (task === 'mediaDistribution' && !posts.length) {
    const type = taskConfig?.contentType || task
    const feed = await fetchSiteFeed(48, { fresh: true, task: type, page }).catch(() => null)
    const unscopedFeed = feed?.posts?.length ? null : await fetchSiteFeed(80, { fresh: true, page }).catch(() => null)
    const sourcePosts = feed?.posts?.length ? feed.posts : (unscopedFeed?.posts || [])
    posts = sourcePosts.filter((post) => {
      ...
      const postType = typeof content.type === 'string' ? content.type : ''
      const postCategory = typeof content.category === 'string' ? normalizeCategory(content.category) : ''
      if (status && status !== 'PUBLISHED') return false
      if (postType && postType !== type && postType !== task) return false
      return category === 'all' || postCategory === category
    }).slice(0, 24)
    pagination = feed?.pagination || unscopedFeed?.pagination || pagination
  }
      ...
      <main className="min-h-screen bg-white text-[#202020]">
        <section className="bg-[#202020] text-white">
          <div className="mx-auto max-w-[1160px] px-4 py-16 text-center sm:px-6 lg:px-8">
            <h1 className="text-4xl font-black uppercase leading-tight sm:text-5xl">{category === 'all' ? label : categoryLabel}</h1>
            <div className="mx-auto mt-4 h-px max-w-3xl bg-white/25" />
            <p className="mx-auto mt-4 max-w-2xl text-sm font-semibold leading-7 text-white/75">Media distribution updates, syndication-ready announcements, and publisher-facing stories organized for fast discovery.</p>
          </div>
        </section>

        <section className="border-b border-black/10 bg-white">
          <div className="mx-auto flex max-w-[1160px] gap-7 overflow-x-auto px-4 py-4 text-xs font-black uppercase tracking-[0.16em] sm:px-6 lg:px-8">
        ...
        <section className="mx-auto grid max-w-[1160px] gap-10 px-4 py-12 sm:px-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:px-8">
          <div>
          {posts.length ? (
            <div className="editable-stagger divide-y divide-black/10">
              {posts.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group block py-10 text-center transition hover:-translate-y-1">
                  <p className="mx-auto w-fit border-b border-black text-xs font-black uppercase">{getCategory(post, label)}</p>
                  <h2 className="mx-auto mt-4 max-w-3xl text-3xl font-black uppercase leading-[1.06] text-[#202020] group-hover:text-[var(--slot4-accent)] sm:text-4xl">{post.title}</h2>
                  
                  <span className="mt-9 inline-flex min-w-60 items-center justify-center rounded-full bg-[#2f2f2f] px-7 py-3 text-sm font-black text-white group-hover:bg-[var(--slot4-accent)]">Continue Reading</span>
            ...
            <div className="mt-10 flex items-center justify-center gap-3">
              {pagination.hasPrevPage ? <Link href={pageHref(basePath, category, page - 1)} className="rounded-full border border-black bg-white px-5 py-3 text-xs font-black uppercase">Previous</Link> : null}
              <span className="rounded-full bg-[#2f2f2f] px-5 py-3 text-xs font-black uppercase text-white">Page {page} / {pagination.totalPages || 1}</span>
              {pagination.hasNextPage ? <Link href={pageHref(basePath, category, page + 1)} className="rounded-full border border-black bg-white px-5 py-3 text-xs font-black uppercase">Next</Link> : null}
            </div>
          </div>
          ...
          <aside className="space-y-8 lg:sticky lg:top-64 lg:self-start">
            ...
            <form action={basePath} className="border border-black/10 bg-white p-8">
              <h2 className="border-b border-black/10 pb-3 text-2xl font-black uppercase">Filter</h2>
              <select name="category" defaultValue={category} className="mt-5 h-12 w-full border border-black/10 bg-white px-3 text-xs font-black uppercase outline-none">
                <option value="all">All categories</option>
                {categories.map((item) => <option key={item.slug} value={item.slug}>{item.name}</option>)}
              </select>
              <button className="mt-3 h-12 w-full bg-[#2f2f2f] text-xs font-black uppercase tracking-[0.14em] text-white">Apply</button>
            </form>
            <div className="border border-black/10 bg-white p-8">
              <h2 className="border-b border-black/10 pb-3 text-2xl font-black uppercase">Recent Posts</h2>
              <div className="mt-5 grid gap-4">
                {recent.map((post) => <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="border-b border-black/10 pb-4 text-sm font-semibold leading-6 hover:text-[var(--slot4-accent)]">{post.title}</Link>)}
              </div>
            </div>
          </aside>
        </section>
